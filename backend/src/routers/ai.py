...
def send_message(
    ...
    try:
        ...
        task_keywords = ["create", "add", "new task", "task:", "todo:", "remind me to", "banaya", "naya", "naye task"]
        wants_to_create_task = any(keyword in content.lower() for keyword in task_keywords)

        # Check if user wants to delete a task
        delete_keywords = ["delete", "remove", "cancel", "khatam", "delete karo", "hataye"]
        wants_to_delete_task = any(keyword in content.lower() for keyword in delete_keywords)

        # Check if user wants to complete/finish a task
        complete_keywords = ["complete", "finish", "done", "mark as done", "mukammal", "purak", "complete kar"]
        wants_to_complete_task = any(keyword in content.lower() for keyword in complete_keywords)

        # Check if user wants to update a task
        update_keywords = ["update", "change", "modify", "edit", "badlo", "update karo"]
        wants_to_update_task = any(keyword in content.lower() for keyword in update_keywords)
        ...
        if wants_to_create_task:
            # Use the AI parser to extract task details
            try:
                from ..ai.parser import parse_task_with_ai
                from ..models import Task

                intent, _ = parse_task_with_ai(
                    user_input=content,
                    user_id=current_user.id,
                    user_timezone="UTC"
                )

                # Create the task if we got a valid title
                if intent.extracted_title:
                    task = Task(
                        user_id=current_user.id,
                        title=intent.extracted_title,
                        priority=intent.extracted_priority or "Medium",
                        completed=False
                    )

                    # Set due date if extracted
                    if intent.extracted_due_date:
                        task.due_date = intent.extracted_due_date
                        if intent.extracted_due_time:
                            # Combine date and time
                            from datetime import time as dt_time
                            due_datetime = intent.extracted_due_date.replace(
                                hour=intent.extracted_due_time.hour,
                                minute=intent.extracted_due_time.minute,
                                second=intent.extracted_due_time.second
                            )
                ...
                logger.warning(f"Failed to auto-create task from chat: {e}")
                # Continue with normal AI response if task creation fails

        elif wants_to_delete_task:
            # Try to find and delete a task based on the user's message
            try:
                from ..models import Task
                import re

                # Extract task number or title from the message
                # Look for patterns like "delete task 1", "delete the first task", "task #1"
                task_number = None
                task_title_pattern = None

                # Try to find a number
                numbers = re.findall(r'\d+', content)
                if numbers:
                    task_number = int(numbers[0])
                ...
                task_to_delete = None
                if task_number and task_number <= len(active_tasks):
                    # User specified a task number
                    task_to_delete = active_tasks[task_number - 1]
                elif not task_number and len(active_tasks) == 1:
                    # Delete the only task if no number specified
                    task_to_delete = active_tasks[0]

                if task_to_delete:
                    deleted_task = {
                        "id": str(task_to_delete.id),
                        "title": task_to_delete.title
                    }
                    db.delete(task_to_delete)
                    db.commit()
                    logger.info(f"Deleted task {task_to_delete.id} via chat")
            except Exception as e:
                logger.warning(f"Failed to delete task from chat: {e}")

        elif wants_to_complete_task:
            # Try to find and complete a task based on the user's message
            try:
                from ..models import Task
                import re

                # Extract task number from the message
                numbers = re.findall(r'\d+', content)
                task_number = None
                if numbers:
                    task_number = int(numbers[0])
                ...
                if task_number and task_number <= len(active_tasks):
                    task_to_complete = active_tasks[task_number - 1]
                elif not task_number and len(active_tasks) == 1:
                    task_to_complete = active_tasks[0]

                if task_to_complete:
                    completed_task = {
                        "id": str(task_to_complete.id),
                        "title": task_to_complete.title
                    }
                    task_to_complete.completed = True
                    db.add(task_to_complete)
                    db.commit()
                    logger.info(f"Completed task {task_to_complete.id} via chat")
            except Exception as e:
                logger.warning(f"Failed to complete task from chat: {e}")

        # Determine user language (use message language or fall back to conversation language)
        user_language = language or conversation.language
        ...
        # Build task context string based on language
        task_context = ""
        if user_tasks:
            ...
            if user_language == "ur":
                task_context = f"\n\nصارف کے موجودہ ٹاسک:\n"
                task_context += f"کل ٹاسک: {len(user_tasks)}\n"
                task_context += f"فعال ٹاسک: {len(active_tasks)}\n"
                task_context += f"مکمل ٹاسک: {len(completed_tasks)}\n\n"

                if active_tasks:
                    task_context += "فعال ٹاسک:\n"
                    for i, task in enumerate(active_tasks[:10], 1):  # Limit to 10 most recent
                        due_str = task.due_date.strftime("%Y-%m-%d %H:%M") if task.due_date else "کوئی تاریخ مقرر نہیں"
                        task_context += f"{i}. {task.title} (ترجیح: {task.priority}, مقررہ تاریخ: {due_str})\n"

                if completed_tasks and len(completed_tasks) <= 5:
                    task_context += f"\nحال ہی میں مکمل ہوئے:\n"
                    for task in completed_tasks[-3:]:
                        task_context += f"✓ {task.title}\n"
            else:
                task_context = f"\n\nUser's Current Tasks:\n"
                task_context += f"Total tasks: {len(user_tasks)}\n"
                task_context += f"Active tasks: {len(active_tasks)}\n"
                task_context += f"Completed tasks: {len(completed_tasks)}\n\n"

                if active_tasks:
                    task_context += "Active Tasks:\n"
                    for i, task in enumerate(active_tasks[:10], 1):  # Limit to 10 most recent
                        due_str = task.due_date.strftime("%Y-%m-%d %H:%M") if task.due_date else "No due date"
                        task_context += f"{i}. {task.title} (Priority: {task.priority}, Due: {due_str})\n"
        ...
        # Construct system prompt with task context and language

        if user_language == "ur":
            # Urdu system prompt
            system_prompt = """آپ ٹاسک پائلٹ ہیں، جو ٹاسک انتظام کے لیے ایک مددگار AI اسسٹنٹ ہیں۔ آپ صارف کی ٹاسک لسٹ تک رسائی رکھتے ہیں اور ان کی مدد کر سکتے ہیں:
- نئے ٹاسک بنائیں
- ٹاسک کو مکمل کروائیں
- ٹاسک کو حذف کریں
- ان کے ٹاسک کی تعداد اور حیثیت چکیں
- آنے والے ٹاسک کے بارے میں یاد دہانی کروائیں
- ٹاسک کو منظم اور ترجیح دیں

جب صارف ان کے ٹاسک کے بارے میں پوچھے، دی گئی ٹاسک معلومات کا استعمال کرتے ہوئے درست جوابات دیں۔
مہربان، مختصر، اور مددگار رہیں۔ آپ کے جوابات ضرور اردو میں ہوں۔"""
        else:
            # English system prompt
            system_prompt = """You are TaskPilot, a helpful AI assistant for task management. You have access to the user's task list and can help them:
- Create new tasks
- Complete tasks
- Delete tasks
- Check their task count and status
- Provide reminders about upcoming tasks
- Help organize and prioritize tasks

When the user asks about their tasks, use the provided task information to give accurate answers.
Be friendly, concise, and helpful."""

        # Construct prompt with context about task operations
        if created_task:
            if user_language == "ur":
                user_prompt = f"صارف نے کہا: '{content}'\n\nمیں نے ایک ٹاسک بنایا ہے اس کی تفصیلات یہ ہیں:\n- عنوان: {created_task['title']}\n- ترجیح: {created_task['priority']}\n- مقررہ تاریخ: {created_task['due_date'] if created_task.get('due_date') else 'نہیں مقرر'}\n\nبراہ کرم صارف کو دوستانہ انداز میں اس کی اطلاع دیں۔"
            else:
                user_prompt = f"User said: '{content}'\n\nI've created a task with the following details:\n- Title: {created_task['title']}\n- Priority: {created_task['priority']}\n- Due Date: {created_task.get('due_date') or 'Not set'}\n\nPlease acknowledge this to the user in a friendly way."
        elif deleted_task:
            if user_language == "ur":
                user_prompt = f"صارف نے کہا: '{content}'\n\nمیں نے اس ٹاسک کو حذف کر دیا ہے: {deleted_task['title']}\n\nبراہ کرم صارف کو دوستانہ انداز میں اس کی اطلاع دیں۔"
            else:
                user_prompt = f"User said: '{content}'\n\nI've deleted this task: {deleted_task['title']}\n\nPlease acknowledge this to the user in a friendly way."
        elif completed_task:
            if user_language == "ur":
                user_prompt = f"صارف نے کہا: '{content}'\n\nمیں نے اس ٹاسک کو مکمل کر دیا ہے: {completed_task['title']}\n\nبراہ کرم صارف کو دوستانہ انداز میں تسلی دیں۔"
            else:
                user_prompt = f"User said: '{content}'\n\nI've marked this task as complete: {completed_task['title']}\n\nPlease congratulate the user in a friendly way."
        else:
            user_prompt = content

        # Add task context to the user prompt
        if task_context:
            if user_language == "ur":
                user_prompt = task_context + "\n\nصارف کا پیغام: " + user_prompt
            else:
                user_prompt = task_context + "\n\nUser message: " + user_prompt

        # Call AI with system prompt and context
        response_text, token_count = client.call(prompt=user_prompt, system=system_prompt)
        ...
        # Return message with task operation info
        response_data = {
            "id": ai_msg.id,
        ...
        if created_task:
            response_data["created_task"] = created_task
        if deleted_task:
            response_data["deleted_task"] = deleted_task
        if completed_task:
            response_data["completed_task"] = completed_task
